 ...
 */
public abstract class StatisticsAbstractSearch extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
    ...
    protected static final String TYPE_STATISTICS_LIST = "statisticsList";
    ...
    public SourceValidity getValidity() {
        if (this.validity == null) {
            try {
                ...
                List<SearchDocument> results = this.queryResults.getAllResults();
...
//            logSearch();
     ...
     *
     * @param statisticsSearch The search division to contain the search-results division.
     */
    protected void buildSearchResultsDivision(Division statisticsSearch)
        ...
        Division results = statisticsSearch.addDivision("statistics-results", "primary");
        buildSearchControls(results);


        DSpaceObject searchScope = getScope();

        //TODO implementar vista de resultados para Statistics
        ...
        {
            displayedResults = queryResults.getAllResults().size();
        ...
        if(queryResults != null && 0 < queryResults.getTotalSearchResults())
        {
            ...
            int lastItemIndex = (int) this.queryResults.getStart() + queryResults.getAllResults().size();
        	...
        	//TODO Agregar resultados en estas listas segun el tipo de estadistica....
        	// Put it on the top of level search result list
        	org.dspace.app.xmlui.wing.element.List statisticsResultsList = results.addList("statistics-search-results-repository", 
        			org.dspace.app.xmlui.wing.element.List.TYPE_SIMPLE, "repository-statistics-search-results");
        	
        	//Aparentemente,el field "UID" es único por estadistica, por eso lo utilizo como identificador del registro...
        	String UID;
        	org.dspace.app.xmlui.wing.element.List searchStatistics = statisticsResultsList.addList(STAT_TYPES.SEARCH.text() + "-results");
        	for (SearchDocument document : queryResults.getResultsByType(STAT_TYPES.SEARCH)) {
        		UID = document.getSearchFieldValues("uid").get(0);
        		addResultFields(document, searchStatistics.addList(UID + ":statistics:" + STAT_TYPES.SEARCH.text()));
			}
        	org.dspace.app.xmlui.wing.element.List viewStatistics = statisticsResultsList.addList(STAT_TYPES.VIEW.text() + "-results");
        	for (SearchDocument document : queryResults.getResultsByType(STAT_TYPES.VIEW)) {
        		UID = document.getSearchFieldValues("uid").get(0);
        		addResultFields(document, viewStatistics.addList(UID + ":statistics:" + STAT_TYPES.VIEW.text()));
			}
        	org.dspace.app.xmlui.wing.element.List workflowStatistics = statisticsResultsList.addList(STAT_TYPES.WFW.text() + "-results");
        	for (SearchDocument document : queryResults.getResultsByType(STAT_TYPES.WFW)) {
        		UID = document.getSearchFieldValues("uid").get(0);
        		addResultFields(document, workflowStatistics.addList(UID + ":statistics:" + STAT_TYPES.WFW.text()));
			}
    ...
    protected void addResultFields(SearchDocument document, org.dspace.app.xmlui.wing.element.List container) throws WingException, SQLException {
    	Map<String, List<String>> fields = document.getSearchFields();
    	for (String fieldKey : document.getSearchFields().keySet()) {
    		org.dspace.app.xmlui.wing.element.List fieldList = container.addList(fieldKey);
    		for (String fieldValue : fields.get(fieldKey)) {
    			fieldList.addItem().addContent(fieldValue);
			}
		}
    	//Agregamos el handle del DSO asociado a la estadística...
    	DSpaceObject dso = StatisticsSearchUtils.getDSOByStatistic(document, context);
    	if(dso != null && dso.getHandle() != null && !dso.getHandle().isEmpty()) {
    		org.dspace.app.xmlui.wing.element.List fieldList = container.addList("handle");
    		fieldList.addItem().addContent(dso.getHandle());
    	}
    }
    ...
    **/
    ...
    //TODO implementar una idea similar pero para hacer un render de cada estadistica
	...
	**/
    ...
    public DiscoverQuery prepareQuery(DSpaceObject scope, String query, String[] fqs) {
    	...
    	//TODO evaluar si es necesario dejarlo para expresiones regulares...
            ...
            if(searchSortConfiguration != null){
                for (DiscoverySortFieldConfiguration sortFieldConfiguration : searchSortConfiguration.getSortFields()) {
                    if(sortFieldConfiguration.equals(searchSortConfiguration.getDefaultSort())){
                        sortBy = sortFieldConfiguration.getMetadataField();
                    }
                }
            }
        }
        ...
        //TODO utilizar LocalParams (https://wiki.apache.org/solr/LocalParams#LocalParams-1)
        //Agregar como defaultFields de busqueda todos los <fields> de Statistics...
        queryArgs.setQuery(StatisticsSearchUtils.generateDefaultFields(query,scope));
	...
	/**
    private String generateDefaultFields(String query, DSpaceObject scope) {
		...
		if(configuration.getDefaultQueryFields().size() > 0) {
			//Utilizamos el Dismax handler para poder usar multiples fields de busqueda por defecto en la consulta, ya que el 'df' solo permite un unico campo...
			builder.append("{! type=dismax qf='");
			for (String queryField : configuration.getDefaultQueryFields()) {
     ...
     */
    public void performSearch(DSpaceObject scope) throws UIException, StatisticsSearchServiceException {
    ...
    protected void buildSearchControls(Division div)
        ...
        {
            currentSort = searchSortConfiguration.getDefaultSort().getMetadataField();
        }
        ...
        if(searchSortConfiguration.getSortFields() != null)
        {
            for (DiscoverySortFieldConfiguration sortFieldConfiguration : searchSortConfiguration.getSortFields())
            {
                String sortField = sortFieldConfiguration.getMetadataField();
