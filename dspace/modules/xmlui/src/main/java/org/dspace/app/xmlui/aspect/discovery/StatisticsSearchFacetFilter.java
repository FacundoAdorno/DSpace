 ...
 */
public class StatisticsSearchFacetFilter extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
    ...
    private StatisticsSearchService statisticsSearchService = null;
	...
	protected String statisticsDiscoveryAspectPath = "statistics-discover";


    public StatisticsSearchFacetFilter() {
    ...
    public SourceValidity getValidity() {
        if (this.validity == null) {
            ...
            try {
                ...
                StatisticsDiscoverResult response = getQueryResponse(dso);

                newValidity.add("numFound:" + response.getAllResults().size());
...
//                }
    ...
    protected StatisticsDiscoverResult getQueryResponse(DSpaceObject scope) {
        ...
        DiscoveryConfiguration discoveryConfiguration = StatisticsSearchUtils.getDiscoveryConfiguration(scope);
        List<String> defaultFilterQueries = discoveryConfiguration.getDefaultFilterQueries();
        queryArgs.addFilterQueries(defaultFilterQueries.toArray(new String[defaultFilterQueries.size()]));


        queryArgs.setQuery(StatisticsSearchUtils.generateDefaultFields(request.getParameter("query"), scope));
...
//            facetFields.addAll(this.queryResults.getFacetDates());
            ...
            if (facetFields.size() > 0) {
                ...
                if (values != null && 0 < values.size()) {
                    ...
                    for (int i = 0; i < end; i++) {
                        StatisticsDiscoverResult.FacetResult value = values.get(i);
    ...
    private void addBrowseJumpNavigation(Division div, SearchFilterParam browseParams, Request request)
        ...
        DSpaceObject dso = HandleUtil.obtainHandle(objectModel);
        if(dso != null){
            action = contextPath + "/handle/" + dso.getHandle() + "/statistics-search-filter";
        }else{
            action = contextPath + "/statistics-search-filter";
        }
        ...
        Map<String, String[]> filterQueries = StatisticsDiscoveryUIUtils.getParameterFilterQueries(request);
        ...
        if(!browseParams.getFacetField().endsWith(".year")){
            ...
            String basicUrl = generateURL("statistics-search-filter", params);
...
//        }
        ...
        if(filterQueries.contains(statisticsSearchService.toFilterQuery(context,  facetField, value.getFilterType(), value.getAsFilterQuery()).getFilterQuery())){
    ...
    private String getNextPageURL(SearchFilterParam browseParams, Request request) throws UnsupportedEncodingException, UIException {
        ...
        String newURL = generateURL("statistics-search-filter", urlParameters);
            ...
            throws WingException
    {
        ...
        String facetField = request.getParameter(SearchFilterParam.FACET_FIELD);
        Division controls = div.addInteractiveDivision("browse-controls", "statistics-search-filter?" + request.getQueryString(),
